...
| `.references(() => table._.columns.column, options?)` | Creates a foreign key via a lazy getter (Drizzle-style). Enables self-references and forward refs. Use `table._.columns.id` to avoid name clashes. For self-refs, add `(): AnySQLiteColumn =>` to fix TS7022. | `integer('author_id').references(() => users._.columns.id, { onDelete: 'cascade' })` |
