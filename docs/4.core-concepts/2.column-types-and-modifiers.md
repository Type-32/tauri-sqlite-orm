...
| `.references(() => table.column, options?)` | Creates a foreign key via a lazy getter (Drizzle-style). Enables self-references and forward refs. Supports `onDelete` and `onUpdate`. | `integer('author_id').references(() => users.id, { onDelete: 'cascade' })` |
